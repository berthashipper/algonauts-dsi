    ...
    for i in tqdm(range(df.shape[0]), desc=f"Extracting {Path(tsv_path).stem}"):
        ...
        if len(tokens) > 0:
            ...
            with torch.no_grad():
                ...
                cls_embedding = outputs.hidden_states[-1][0][0].cpu().numpy()  # [CLS] token
                pooler_output.append(cls_embedding)
        else:
        ...
        if len(np_tokens) > 0:
            ...
            with torch.no_grad():
                outputs = model(np_tensor, output_hidden_states=True, return_dict=True)
                np_out = outputs.hidden_states[-1][0][1:-1].cpu().numpy()
...
if __name__ == "__main__":
    ...
    save_dir_features = "../results/modern_language_features"
